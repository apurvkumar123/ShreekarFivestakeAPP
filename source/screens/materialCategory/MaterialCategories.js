...
const MaterialCategories = () => {
  ...
  function moveToNext(item) {
    navigation.navigate(stack.ADD_MATERIAL_CAT, {
  ...
  async function requestGetAdminData() {
    ...
    console.log('REQ_getMaterialCategories', data);
    return Axios.requestData('POST', apiName.MaterialCategoryList, data)
      .then(res => {
        console.log(
          'RES_getMaterialCategories',
          res.data.data.MaterialCategorylist,
        );
        setIsScreenLoading(false);
        setRefreshing(false);
        if (res.data.status == true) {
          setAdminUserData(res.data.data.MaterialCategorylist);
          setIsScreenLoading(false);
          setRefreshing(false);
        } else {
          setAdminUserData([]);
  ...
  return (
    <SafeAreaView style={commonStyle.commonBackgroundStyle}>
      ...
      <View style={styles.safeAreaBaseViewContainer}>
        <Header
          route={strings.lbl_menu_materialCat}
          isIconDisplay={true}
          nav={navigation}
          isBackShow={true}
        />
        {adminUserData != null ? (
          <Text style={styles.topTotalbLable}>
            {adminUserData.length + ' ' + strings.lbl_menu_materialCat}
          </Text>
        ) : null}
