...
const AddMaterialCat = ({route}) => {
  ...
  function validation() {
    if (fname.trim().length == 0) {
      showFlashMessage('Required', strings.msg_materialcat_name, true);
  ...
  async function requestAddAdminUser(data) {
    console.log('REQ_AddMaterialCat', data);
    return Axios.requestData(
      'POST',
      isAdd ? apiName.AddMaterialCat : apiName.EditMaterialCat,
      data,
    )
      .then(res => {
        setIsLoading(false);
        if (res.data.status == true) {
          console.log('RES_AddMaterialCat', res.data.data);
  ...
  async function requestDelete() {
    ...
    return Axios.requestData('POST', apiName.DeleteMaterialCat, data)
      .then(res => {
        setIsLoading(false);
    ...
    <SafeAreaView style={commonStyle.commonBackgroundStyle}>
      <View style={styles.safeAreaBaseViewContainer}>
        ...
        <Animated.View
          ...
          <Header
            route={
              isAdd ? strings.lbl_add_materialCat : strings.lbl_edit_materialCat
            }
            isIconDisplay={true}
            nav={navigation}
            isBackShow={true}
          ...
          <View style={[styles.topContainer]}>
            ...
            <ScrollView style={{marginHorizontal: 5}}>
              <InputField
                ...
                isEditable={true}
                isImage={false}
                inputTitle={strings.lbl_material_category}
