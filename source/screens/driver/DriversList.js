...
const DriversList = () => {
  ...
  function moveToNext(item) {
    navigation.navigate(stack.ADD_DRIVER, {
  ...
  async function requestGetAdminData() {
    ...
    console.log('REQ_getDriversList', data);
    return Axios.requestData('POST', apiName.getDriversList, data)
      .then(res => {
        console.log('RES_getDriversList', res.data.data);
        setIsScreenLoading(false);
        setRefreshing(false);
        if (res.data.status == true) {
          setAdminUserData(res.data.data.driverlist);
          setIsScreenLoading(false);
          setRefreshing(false);
        } else {
          setAdminUserData([]);
      ...
      .catch(err => {
        ...
        setRefreshing(false);
        setAdminUserData([]);
        console.log('RES_getDriversList_err', err);
      });
  }
      ...
      <View style={styles.safeAreaBaseViewContainer}>
        ...
        {adminUserData != null ? (
          <Text style={styles.topTotalbLable}>
            {adminUserData.length + ' ' + strings.lbl_menu_driver + '(s)'}
          </Text>
        ) : null}
