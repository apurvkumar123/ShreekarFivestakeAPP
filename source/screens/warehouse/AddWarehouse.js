...
const AddWarehouse = ({route}) => {
  ...
  function validation() {
    if (fname.trim().length == 0) {
      showFlashMessage('Required', strings.msg_warehouse_name, true);
  ...
  async function requestAddAdminUser(data) {
    console.log('REQ_AddWare', data);
    return Axios.requestData(
      'POST',
      isAdd ? apiName.AddWarehouse : apiName.EditWarehouse,
      data,
    )
      .then(res => {
        setIsLoading(false);
        if (res.data.status == true) {
          console.log('RES_AddWare', res.data.data);
  ...
  async function requestDelete() {
    ...
    return Axios.requestData('POST', apiName.DeleteWarehouse, data)
      .then(res => {
        setIsLoading(false);
    ...
    <SafeAreaView style={commonStyle.commonBackgroundStyle}>
      <View style={styles.safeAreaBaseViewContainer}>
        ...
        <Animated.View
          ...
          <Header
            route={
              isAdd ? strings.lbl_add_warehouse : strings.lbl_edit_warehouse
            }
            isIconDisplay={true}
            nav={navigation}
            isBackShow={true}
          ...
          <View style={[styles.topContainer]}>
            ...
            <ScrollView style={{marginHorizontal: 5}}>
              <InputField
                ...
                isEditable={true}
                isImage={false}
                inputTitle={strings.lbl_warehouse_name_hint}
