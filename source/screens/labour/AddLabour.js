...
const AddLabour = ({route}) => {
  ...
  function importData() {
    ...
    setAdminId(item.Id);
    setFname(item.LabourName);
    setLname(item.AadharNo);
    setCity(item.CityName);
    setNoofPerson('' + item.NoOfPerson);
  ...
  function validation() {
    if (fname.trim().length == 0) {
      showFlashMessage('Required', strings.msg_labour_name, true);
    } else if (lname.trim().length == 0) {
      showFlashMessage('Required', strings.msg_aadhar_no, true);
    } else if (city.trim().length == 0) {
      showFlashMessage('Required', strings.msg_enter_city, true);
    } else if (noofPerson.trim().length == 0) {
      showFlashMessage('Required', strings.msg_no_person, true);
  ...
  async function requestAddAdminUser(data) {
    ...
    return Axios.requestData(
      'POST',
      isAdd ? apiName.AddLabour : apiName.EditLabour,
      data,
    )
      .then(res => {
        setIsLoading(false);
  ...
  async function requestDelete() {
    ...
    return Axios.requestData('POST', apiName.DeleteLabour, data)
      .then(res => {
        setIsLoading(false);
    ...
    <SafeAreaView style={commonStyle.commonBackgroundStyle}>
      <View style={styles.safeAreaBaseViewContainer}>
        ...
        <Animated.View
          ...
          <Header
            route={isAdd ? strings.lbl_add_labour : strings.lbl_edit_labour}
            isIconDisplay={true}
            nav={navigation}
            isBackShow={true}
          />
          ...
          <View style={[styles.topContainer]}>
            ...
            <ScrollView style={{marginHorizontal: 5}}>
              ...
              <InputField
                ...
                isEditable={true}
                isImage={false}
                inputTitle={strings.lbl_aadhar_no_hint}
              ...
              <InputField
                ...
                isEditable={true}
                isImage={false}
                inputTitle={strings.lbl_no_of_person}
                updateMasterState={value => setNoofPerson(value)}
                value={noofPerson}
                returnKey={constant.KEY_BTN_NEXT}
              />
              <InputField
