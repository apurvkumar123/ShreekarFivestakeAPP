...
const AddProjectFundKishanBhai = ({route}) => {
  ...
  async function requestAddAdminUser(data) {
    ...
    return Axios.requestData(
      'POST',
      isAdd ? null : apiName.actionProjectFund,
      data,
    )
      .then(res => {
        setIsLoading(false);
  ...
  async function requestGetProject() {
    ...
    var data = {
      createdBy: 0,
      type: 1, //1 -Kishanbhai , 2 - Dineshbhai
      languageId: constant.LANGUAGE_ID,
    };
    ...
    return Axios.requestData('POST', apiName.getProjectFUndDetails, data)
      .then(res => {
        ...
        if (res.data.status == false) {
          ...
          if (
            detailedRes.RequestStatus === 2 ||
            detailedRes.RequestStatus === 4 ||
  ...
  const isDisabled = [3, 5, 7].includes(statusI);
  function balanceUI() {
    return (
      <View
        style={{
          ...
          <View style={[styles.topContainer]}>
            ...
            <ScrollView style={{marginHorizontal: 5}}>
              ...
              <InputField
                ...
                isImage={false}
                inputTitle={strings.lbl_f_amount_hint}
...
export default AddProjectFundKishanBhai;
