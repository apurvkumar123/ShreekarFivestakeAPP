...
const AddProjectFundAccountPerson = ({route}) => {
  ...
  async function requestAddAdminUser(data) {
    ...
    return Axios.requestData(
      'POST',
      isAdd ? null : apiName.actionAccountProjectFund,
      data,
    )
      .then(res => {
        setIsLoading(false);
  ...
  async function requestGetProject() {
    ...
    var data = {
      createdBy: 0,
      type: mode == strings.lbl_credit_mode ? 3 : 4, //1 -Kishanbhai , 2 - Dineshbhai, 3 - Account Person(released), 4 - Account Person(received),
      languageId: constant.LANGUAGE_ID,
    };
  ...
  const isReceivedMode = mode === strings.lbl_debit_mode;
  function balanceUI() {
    return (
      <View
        style={{
...
export default AddProjectFundAccountPerson;
